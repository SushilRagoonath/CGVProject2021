...
function createHealthBoxes(){
    health =[];
    healthBoxes=[];
    loader.load('../assets/health/health_pack.glb',function(glb){
        console.log('health loaded')
        healthBox = glb;
        
        healthBox.scene.scale.set(1,1,1)
        for (let index = 0; index < healthNumber; index++) {
                let x = 70 * Math.random() 
                let z = 30* Math.random()  + 450 *(index+1) 
                let y = 5 * Math.random()
                healthBox.scene.traverse((o) => {
                ...
                healthBox.scene.position.x = x
                healthBox.scene.position.y = y
                healthBox.scene.position.z = z
                //creates hitbox for each healthbox
                let box = new THREE.Box3().setFromObject(healthBox.scene)
                healthBoxes.push(box)
                scene.add(healthBox.scene)
            }       

        
    })
    /*const healthGeometry = new THREE.BoxGeometry(6,6,6);
    //const healthMaterial = new THREE.MeshLambertMaterial({color:0xFF69B4});
    for (let index = 0; index < healthNumber; index++) {
        let x = 70 * Math.random() 
        let z = 30* Math.random()  + 450 *(index+1) 
        let y = 5 * Math.random()
        healthBox = new THREE.Mesh( healthGeometry, healthMaterial );
